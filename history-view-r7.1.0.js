    ...
    <div class="hist-filters">
      ...
      <div class="hist-filter-row">
        ...
        <select id="history-type-select">
          ...
          <option value="status">入出庫・棚卸 / Check-in · Check-out · Audit</option>
        </select>
      </div>
      <div class="hist-filter-row">
...
`;
    ...
    ensureHistoryEventsBuilt() {
      ...
      this.buildHistoryEvents(
        dm.data.locationlog || [],
        dm.data.shiplog || [],
        dm.data.statuslogs || [],     // NEW: thêm bảng statuslogs từ DataManager
        dm.data.molds || [],
        dm.data.cutters || [],
        dm.data.companies || [],
        dm.data.employees || []
    ...
    loadHistoryFromGithub() {
      ...
      const urls = {
        ...
        status: base + 'statuslogs.csv',   // NEW: statuslogs
      ...
      Promise.all([
        ...
        fetchText(urls.ship),
        fetchText(urls.status),            // NEW: status
        fetchText(urls.molds),
        fetchText(urls.cutters),
        fetchText(urls.companies),
        fetchText(urls.employees)
      ])
        .then(([locationText, shipText, statusText, moldsText, cuttersText, companiesText, employeesText]) => {
          const locationlog = parseCsv(locationText);
          const shiplog = parseCsv(shipText);
          const statuslogs = parseCsv(statusText);   // NEW: parse statuslogs
          ...
          this.buildHistoryEvents(
            locationlog,
            shiplog,
            statuslogs,   // NEW: truyền statuslogs vào
            molds,
            cutters,
            companies,
            employees
    ...
    buildHistoryEvents(locationlog, shiplog, statuslogs, molds, cutters, companies, employees) {
      ...
      // 3) Statuslogs → Check-in / Check-out / Audit / ghi chú trạng thái
      (statuslogs || []).forEach(row => {
        const moldIdRaw = row.MoldID;
        const cutterIdRaw = row.CutterID;
        const hasMold = moldIdRaw && String(moldIdRaw).trim() !== '';
        const hasCutter = cutterIdRaw && String(cutterIdRaw).trim() !== '';
        const itemTypeRaw = String(row.ItemType || '').toLowerCase();
        const itemType = hasMold
          ? 'mold'
          : (hasCutter ? 'cutter' : (itemTypeRaw || 'unknown'));
        const itemId = hasMold
          ? String(moldIdRaw)
          : (hasCutter ? String(cutterIdRaw) : '');

        let itemCode = '';
        let itemName = '';
        if (itemType === 'mold' && hasMold) {
          const m = moldById.get(String(moldIdRaw));
          if (m) {
            itemCode = m.MoldCode || m.MoldID || '';
            itemName = m.MoldName || '';
          }
        } else if (itemType === 'cutter' && hasCutter) {
          const c = cutterById.get(String(cutterIdRaw));
          if (c) {
            itemCode = c.CutterNo || c.CutterCode || c.CutterID || '';
            itemName = c.CutterName || c.Name || '';
        ...
        const auditTypeRaw = String(row.AuditType || '').toUpperCase().trim();
        const notesRaw = row.Notes || '';
        let eventType = 'status-log';
        let eventTypeJa = 'ステータス';
        let eventTypeVi = 'Ghi chú trạng thái';

        if (statusRaw === 'CHECK_IN') {
          if (auditTypeRaw === 'AUDIT' || notesRaw.indexOf('棚卸') !== -1) {
            // Kiểm kê / audit
            eventType = 'audit';
            eventTypeJa = '棚卸 / Audit';
            eventTypeVi = 'Kiểm kê';
          } else {
            eventType = 'check-in';
            eventTypeJa = '入庫 / チェックイン';
            eventTypeVi = 'Check-in';
          }
        } else if (statusRaw === 'CHECK_OUT') {
          eventType = 'check-out';
          eventTypeJa = '出庫 / チェックアウト';
          eventTypeVi = 'Check-out';
        }

        const dateStr = row.Timestamp || row.AuditDate || '';
        const empId = row.EmployeeID || row.EmployeeId || '';
        let handlerName = '';
        if (empId && employeeById.has(String(empId))) {
          const emp = employeeById.get(String(empId));
          handlerName = emp.EmployeeNameShort || emp.EmployeeName || '';
        }

        const destId = row.DestinationID || row.DestinationId || '';
        // Không có master chi tiết, nên chỉ đưa DestID vào tên đích hoặc notes
        const destLabel = destId ? ('DestID:' + destId) : '';

        const mergedNotes = notesRaw; // giữ nguyên ghi chú người dùng

        events.push({
          EventID: 'ST' + (row.StatusLogID || ''),
          Source: 'statuslogs',
          EventType: eventType,
          EventTypeLabelJa: eventTypeJa,
          EventTypeLabelVi: eventTypeVi,
          ...
          ToCompanyID: destId || '',
          FromCompanyName: '',
          ToCompanyName: destLabel,  // sẽ hiển thị dạng "- → DestID:xx"
          Direction: eventType === 'check-out' ? 'OUT' : 'IN',
          Notes: mergedNotes || '',
          HandlerID: empId || '',
          Handler: handlerName || ''
        });
    ...
    applyFilters() {
      ...
      const filtered = all.filter(ev => {
        ...
        if (typeFilter === 'rack') {
          ...
          if (!['check-in', 'check-out', 'audit', 'status-log'].includes(ev.EventType)) return false;
        }
