  ...
  const ShippingManager = {
    ...
    openModal(item) {
      ...
      const currentStorageId =
        item.storageCompanyId ||
        item.storage_company ||
        item.storage_companyId ||
        '';
      ...
      // Gắn bàn phím ảo / focus input đầu
      setTimeout(() => {
        const firstInput =
          document.querySelector('#ship-panel input, #ship-panel textarea');
        if (firstInput) {
          firstInput.focus();
          document.dispatchEvent(
            new CustomEvent('keyboardattach', {
    ...
    close() {
      ...
      // Khôi phục focus cho ô tìm kiếm chính
    ...
    renderHistory(logs, companies, employees) {
      ...
      const rows = logs
        .map((l) => {
          ...
          return `
<tr>
  <td data-time="${this.escapeHtml(
    l.DateEntry || l.ShipDate || ''
  )}">${this.escapeHtml(time)}</td>
...
<table class="history-table" id="ship-his">
  ...
  <tbody>
    ...
    initSearchableSelects({ companies, employees, defaultFromId }) {
      ...
      const hasSearchable =
        typeof window.createSearchableSelect === 'function';

      // FROM company
      const fromContainer = document.getElementById(
        'ship-from-select-container'
      );
      if (fromContainer && hasSearchable) {
        ...
        if (defaultFromId) {
          if (typeof fromSelect.setValue === 'function') {
            ...
            if (input) {
              input.dataset.selectedId = defaultFromId;
            }
          }
        }
      }
      ...
      // Employee
      const empContainer = document.getElementById(
        'ship-employee-select-container'
      );
      if (empContainer && hasSearchable) {
        ...
        if (employees && employees.length > 0) {
          ...
          if (typeof empSelect.setValue === 'function') {
            ...
            if (input) {
              input.dataset.selectedId = firstId;
            }
          }
        }
      }
    ...
    bindModalEvents(item, companies, employees) {
      ...
      try {
        ...
        const fromCompanyId = (
          (fromInput && (fromInput.dataset.selectedId || fromInput.value)) ||
          ''
        ).trim();

        const toCompanyId = (
          (toInput && (toInput.dataset.selectedId || toInput.value)) ||
          ''
        ).trim();

        const empId = (
          (empInput && (empInput.dataset.selectedId || empInput.value)) ||
          ''
        ).trim();
        ...
        // ===== ShipID mới =====
        const dmData =
          window.DataManager && window.DataManager.data
        ...
        // === Tạo log CHECK_OUT cho statuslogs.csv (checkout do ship ra ngoài công ty)
        const statusEntry = {
          StatusLogID: '',                           // để trống, backend sẽ gán nếu cần
          MoldID: moldId || '',
          CutterID: cutterId || '',
          ItemType: isMold ? 'mold' : 'cutter',
          Status: 'CHECK_OUT',                       // cùng format với CheckInOut
          Timestamp: nowIso,                         // 2025-12-04T09:49:10.124Z ...
          EmployeeID: empId || '',
          DestinationID: '',                         // chưa map sang destinations
          // Ghi chú: nêu rõ là xuất kho do vận chuyển sang công ty khác
          Notes: `出荷（他社へ移動）/ Vận chuyển ra công ty khác: ${fromName || 'YSD'} → ${toName || ''}`,
          // AuditDate: dùng ngày ship (yyyy-mm-dd) giống các dòng AUDIT trong file
          AuditDate: shipDate || nowIso.split('T')[0],
          // AuditType: đặt riêng cho case vận chuyển ra ngoài
          AuditType: 'SHIP-TO-COMPANY',              // phong cách UPPER_CASE giống AUDIT_ONLY, AUDIT-WITH-RELOCATION
        };


        // ===== ĐÓNG UI NGAY LẬP TỨC =====
        this.close(); // đóng panel Vận chuyển
        ...
        // ===== 2. Ghi statuslogs.csv (CHECK_OUT) =====
        try {
        ...
        if (!updRes.ok || !updJson.success) {
          throw new Error(
            updJson.message || 'Không cập nhật được nơi lưu trữ.'
          );
        }
        ...
        if (data) {
          ...
          if (isMold) {
            ...
            const cutter = data.cutters.find(
              (c) =>
                String(c.CutterID || '').trim() ===
                String(cutterId).trim()
            );
            if (cutter) {
          ...
          if (isMold) {
            updatedItem = data.molds.find(
              (m) =>
                String(m.MoldID || '').trim() === String(moldId).trim()
            );
          } else {
            updatedItem = data.cutters.find(
              (c) =>
                String(c.CutterID || '').trim() ===
                String(cutterId).trim()
            );
          ...
          if (updatedItem) {
            ...
            // Cập nhật detail panel + badge IN/OUT (UIRenderer & MobileDetailModal)
            document.dispatchEvent(
        ...
        this.showBilingualToast('error', err && err.message);
        alert(
          'Lỗi khi ghi dữ liệu vận chuyển.\n' +
            '出荷データの保存中にエラーが発生しました。\n\n' +
    ...
    getCompanyName(companyId, companies) {
      ...
      const c = list.find(
        (x) =>
          String(x.CompanyID || '').trim() === String(companyId).trim()
      );
      if (!c) return companyId;
      return (
        c.CompanyShortName ||
        c.CompanyName ||
        `ID:${c.CompanyID || companyId}`
      );
    },
    ...
    getEmployeeName(empId, employees) {
      ...
      const e = list.find(
        (x) =>
          String(x.EmployeeID || '').trim() === String(empId).trim()
      );
      if (!e) return empId;
    ...
    showBilingualToast(type, extraMessage) {
      let message = '';
      if (type === 'success') {
        message = '出荷を登録しました。 / Ghi nhận vận chuyển thành công.';
      } else if (type === 'error') {
        message =
          '出荷データの保存に失敗しました。 / Lỗi ghi dữ liệu vận chuyển.';
        if (extraMessage) {
      ...
      this.showToast(message, type === 'error' ? 'error' : type);
    },
