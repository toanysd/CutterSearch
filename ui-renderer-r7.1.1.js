...
(function () {
  ...
  const UIRenderer = {
    ...
    init() {
      ...
      // R7.1.1-FIX: CHỈ sort, KHÔNG đụng vào category
      document.addEventListener('results:sortChanged', (e) => {
        ...
        // ✅ R7.1.1-FIX: CHỈ cập nhật sort config, KHÔNG đụng category
      ...
      document.addEventListener('category:changed', (e) => {
        ...
        // Re-render với danh sách đã lọc (GIỮ NGUYÊN SORT)
    ...
    renderQuickCards(items, append = false) {
      ...
      itemsToRender.forEach((item, localIdx) => {
        ...
        if (itemLogs.length > 0) {
          displayDate = itemLogs[0].Timestamp;
        }
        if (!displayDate) {
    ...
    setupInfiniteScroll() {
      ...
      // ✅ Đã setup rồi thì skip
      if (wrap.dataset.infiniteScrollSetup === 'true') return;

      const scrollHandler = throttle(() => {
        // ✅ Kiểm tra nếu đang load hoặc đã hết items
    ...
    setupPullToRefresh() {
      ...
      if (!wrap || !searchInput) {
        console.warn('[UIRenderer] ⚠️ Pull-to-refresh: missing container or search input');
        return;
      }
    ...
    /**
     * ✅ R7.1.2: Setup card click events (delegation)
     * Gộp tất cả logic click vào 1 handler duy nhất
     */
    setupCardEvents(wrap) {
    if (wrap.dataset.eventsSetup === 'true') return;

    // ✅ CRITICAL FIX: Delegation checkbox click - Higher priority
    wrap.addEventListener('click', (e) => {
      // Tìm checkbox (bao gồm cả khi click vào parent)
      const checkboxIcon = e.target.closest('.inv-bulk-checkbox');
      if (checkboxIcon) {
        console.log('[UIRenderer] 🎯 Checkbox clicked!');
        ...
        if (!card) {
          console.warn('[UIRenderer] Card not found for checkbox');
          return;
        }
        ...
        if (!id) {
          console.warn('[UIRenderer] Card has no ID');
          return;
        }

        // ✅ Toggle visual state IMMEDIATELY (không đợi SelectionManager)
        const isCurrentlySelected = card.classList.contains('inv-selected');
        
        if (isCurrentlySelected) {
          // Deselect
          card.classList.remove('inv-selected', 'inv-bulk-selected');
          checkboxIcon.classList.remove('checked');
          console.log('[UIRenderer] ✅ Deselected:', id);
        } else {
          // Select
          card.classList.add('inv-selected', 'inv-bulk-selected');
          checkboxIcon.classList.add('checked');
          console.log('[UIRenderer] ✅ Selected:', id);
        }

        // ✅ Update SelectionManager state
        if (window.SelectionManager && typeof window.SelectionManager.toggleItem === 'function') {
          const index = parseInt(card.getAttribute('data-index'), 10);
          const itemData = (!isNaN(index) && this.state.allResults[index])
            ? this.state.allResults[index]
            : null;
          
          window.SelectionManager.toggleItem(id, type, itemData);
        }

        return; // Stop propagation
      }

      // ✅ Delegation: click vào card
      ...
      // MODE 1: Selection mode ON → toggle chọn
      if (isSelectionMode) {
        e.preventDefault();
        e.stopPropagation();
        if (window.SelectionManager && typeof window.SelectionManager.toggleItem === 'function') {
      ...
      // MODE 2: Xem chi tiết
      // Mobile: dùng MobileDetailModal
      if (window.innerWidth <= 1024 && window.MobileDetailModal) {
        e.preventDefault();
        e.stopPropagation();

        const index = parseInt(card.getAttribute('data-index'), 10);
        const item = !isNaN(index) ? this.state.allResults[index] : null;
      ...
      // Desktop: phát event detail:changed
      const index = parseInt(card.getAttribute('data-index'), 10);
      const item = !isNaN(index) ? this.state.allResults[index] : null;
    ...
    console.log('[UIRenderer] ✅ Card events setup complete');
  },
