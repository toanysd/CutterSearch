/**
 * =============================================================================
 * history-view-r7.1.7.js
 *
 * Unified History View Module - Version r7.1.7
 *
 * CHANGELOG r7.1.7 (2025-12-18 21:33 JST):
 * ✅ FIX: Panel layout flash on mobile (direct fullscreen, no transition)
 * ✅ FIX: Default show ALL data (no date filter on initial load)
 * ✅ FEATURE: Smart date search (20251218, 2025/12/18, 1218, 202512, 2025/12)
 * ✅ FEATURE: Auto-refresh on open (always fetch latest data)
 * ✅ FEATURE: Notification system (出 red, 位 orange, highlight unread rows)
 * ✅ FEATURE: Mark as read on click (localStorage tracking like Facebook)
 * ✅ UI: Compact mobile filter layout (3 rows: Date+Apply | Action+Employee | Search+Field+Value)
 * ✅ UI: Footer buttons in single row (Close | CSV | Print | Mail)
 * ✅ UI: Reduced padding to show more rows on mobile
 * ✅ KEEP: All r7.1.6 features (swipe to close, safe area, etc.)
 *
 * Dependencies:
...
(function() {
  ...
  // ✅ NEW: Notification tracking (7 days for "new" data)
  ...
  // ✅ NEW: Smart date search - Support multiple formats
  ...
  // ✅ NEW: Check if event is within notification window (last 7 days)
  ...
  // ✅ NEW: LocalStorage helpers for read tracking
  ...
  const HistoryView = {
    ...
    init() {
      ...
      console.log('[HistoryView r7.1.7] Initializing...');
      ...
      // ✅ NEW: Don't apply default date range - show ALL data
      // this.applyDefaultDateRange(); // REMOVED
      ...
      console.log('[HistoryView r7.1.7] Initialized');
    },
    ...
    buildHistoryEvents(locationlog, shiplog, statuslogs, molds, cutters, companies, employees, destinations) {
      ...
      // ✅ NEW: Update IsRead status from localStorage
      const readEvents = getReadEvents();
      events.forEach(ev => {
      ...
      console.log('[HistoryView r7.1.7] events built:', events.length);
      ...
      // ✅ NEW: Update navbar badge
      this.updateNavBadge();
    },

    // =========================================================================
    // ✅ NEW: UPDATE NAVBAR BADGE (出 red, 位 orange) - UNREAD ONLY
    // =========================================================================
    updateNavBadge() {
      const btns = document.querySelectorAll('[data-history-view-trigger]');
      if (!btns || btns.length === 0) return;

      // ✅ Count UNREAD recent events
      const unreadShipOut = this.state.allEvents.filter(ev => 
        ev.ActionKey === ACTION.SHIP_OUT && ev.IsRecent && !ev.IsRead
      ).length;
      
      const unreadLocation = this.state.allEvents.filter(ev => 
        ev.ActionKey === ACTION.LOCATION_CHANGE && ev.IsRecent && !ev.IsRead
      ).length;
      ...
      console.log('[HistoryView] Badge updated:', { unreadShipOut, unreadLocation });
    },

    // =========================================================================
    // CREATE MODAL HTML - ✅ NEW COMPACT MOBILE LAYOUT
    // =========================================================================
    createModal() {
      const html = `
<div id="history-view-root" class="hist-root">
  ...
  <div class="hist-dialog" id="history-dialog">
    ...
    <div class="hist-body">
      ...
      <!-- ✅ NEW COMPACT MOBILE FILTERS - 3 ROWS -->
      <div class="hist-filters" id="history-filters">
        
        <!-- ✅ ROW 1: Date From | Date To | Apply Button (3 columns) -->
        ...
        <!-- ✅ ROW 2: Action Type | Employee (2 columns) -->
        ...
        <!-- ✅ ROW 3: Search (highlighted) | Field | Value (3 columns) -->
        <div class="hist-filter-row-3">
          <!-- ✅ Search field - highlighted with border -->
    ...
    <!-- ✅ FOOTER: 4 buttons in single row -->
...
`;
    ...
    cacheDOMElements() {
      ...
      this.els.applyDateBtn = document.getElementById('history-apply-date-btn'); // ✅ NEW
    ...
    bindInsideEvents() {
      ...
      // ✅ NEW: Apply Date button - Enable date filter
    ...
    open(preset) {
      ...
      // ✅ NEW: Always refresh data on open
    ...
    applyPreset(preset) {
      ...
      this.state.dateFilterEnabled = false; // ✅ Disable date filter
    ...
    // =========================================================================
    // CLEAR FILTERS - ✅ CLEAR DATE FILTER TOO
    // =========================================================================
    clearFilters() {
    ...
    applyFiltersAndRender(resetPage) {
      ...
      let filtered = this.state.allEvents.filter(ev => {
        // ✅ NEW: Date range - only if enabled
        ...
        // ✅ NEW: Smart keyword search (including date formats)
        if (keyword) {
    ...
    toggleSort(key) {
      if (this.state.sortKey === key) {
        ...
        this.state.sortDir = 'desc'; // Default to descending (newest first)
      }
    ...
    renderTable() {
      ...
      pageEvents.forEach(ev => {
        ...
        // ✅ NEW: Check if unread (recent + not read + specific actions)
    ...
    // =========================================================================
    // ✅ SEND MAIL - UPDATED WITH toan@ysd-pack.co.jp
    // =========================================================================
    sendMail() {
      if (!this.state.filteredEvents || this.state.filteredEvents.length === 0) {
        alert('データがありません / Không có dữ liệu');
  ...
  console.log('[HistoryView r7.1.7] Module loaded');

})();

/**
 * =============================================================================
 * CHANGELOG r7.1.7 (2025-12-18 21:50 JST)
 * =============================================================================
 * 
 * FIXES:
 * ------
 * ✅ Panel layout flash on mobile: Direct fullscreen, no CSS transition delay
 * ✅ Default show ALL data: Removed date filter on initial load
 * 
 * FEATURES:
 * ---------
 * ✅ Smart date search: Support formats 20251218, 2025/12/18, 1218, 202512, 2025/12
 * ✅ Auto-refresh on open: Always fetch latest data via DataManager.loadAllData()
 * ✅ Notification system:
 *    - Badge 出 (red) for SHIP_OUT (unread recent events in last 7 days)
 *    - Badge 位 (orange) for LOCATION_CHANGE (unread recent events)
 *    - Highlight unread rows with yellow background (.hist-row-unread)
 *    - Click row → mark as read → remove highlight + update badge
 *    - LocalStorage tracking (key: historyview_read_events)
 * ✅ Date filter apply button: Only apply date range when "適用" button clicked
 * 
 * UI IMPROVEMENTS:
 * ----------------
 * ✅ Compact mobile filter layout:
 *    - Row 1: From Date | To Date | Apply (3 cols)
 *    - Row 2: Action | Employee (2 cols)
 *    - Row 3: Search (highlighted) | Field | Value (3 cols)
 * ✅ Footer buttons: Single row (Close | CSV | Print | Mail)
 * ✅ Reduced padding: Show more rows on mobile
 * ✅ Search input highlighted: Border style to draw attention
 * 
 * PRESERVED:
 * ----------
 * ✅ All r7.1.6 features:
