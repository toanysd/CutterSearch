/**
 * ========================================================================
 * teflon-manager-r7.2.2.js - Quản lý mạ Teflon (ULTIMATE VERSION)
 * ========================================================================
 *
 * Based on: r7.2.1 (scroll fixed) + r7.1.5 (all features)
 *
 * INTEGRATED in r7.2.2 (2025-12-18 08:57 JST):
 * ✅ Vertical scroll fix from r7.2.1 (flex:1;overflow:auto)
 * ✅ Nav badge with kanji from r7.1.5 (発/承/中)
 * ✅ Horizontal scroll lock from r7.1.5 (table-locked + col-hidden-locked)
 * ✅ CSV export with BOM from r7.2.1 (Japanese support)
 * ✅ Filter layout 2-line from r7.1.5 (better UX)
 * ✅ All button behaviors and layouts
 * ✅ Process manager integration
 * ✅ Performance optimization
 * ✅ CSS reference: teflon-manager-r7.2.2.css (FIXED)
 *
 * Dependencies:
 * - window.DataManager (molds, teflonlog, employees)
 * - window.TeflonProcessManager (optional)
 *
 * Updated: 2025-12-18 08:57 JST
 * ========================================================================
 */
...
(function () {
  ...
  let isTableLocked = true; // ✅ r7.1.5 feature
  ...
  // ========================================================================
  function buildRows() {
    console.log('[TeflonManager r7.2.2] buildRows called');
  ...
  function injectStyles() {
    if (document.getElementById('teflon-manager-r7-2-2-styles')) return;

    const link = document.createElement('link');
    link.id = 'teflon-manager-r7-2-2-styles';
    link.rel = 'stylesheet';
    link.href = 'teflon-manager-r7.2.2.css'; // ✅ FIXED: Correct CSS file reference
    document.head.appendChild(link);
  }
  // ========================================================================
  // RENDER TABLE (r7.1.5 style with clickable links)
  // ========================================================================
  function renderTable() {
    ...
    filteredRows.forEach(row => {
      ...
      // ✅ r7.1.5 style: clickable mold name + status badge
      html += `
...
<td class="col-hidden-locked" style="padding:8px 10px;max-width:200px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;">${escapeHtml(notes)}</td>
    ...
    // ✅ r7.1.5 style: separate handlers for mold name and status badge
  ...
  // ========================================================================
  // PROCESS MANAGER INTEGRATION (r7.2.1)
  // ========================================================================
  ...
  function openProcessManager(row) {
    ...
    // ✅ Try r7.1.5 style first (openPanel)
    ...
    // ✅ Try r7.2.1 style (openModal with retry)
...
<div class="vi">Đóng</div>
  ...
  // ========================================================================
  // NAV BADGE UPDATE (r7.1.5: KANJI CHARACTERS 発/承/中)
  // ========================================================================
  ...
  // ========================================================================
  // EXPORT / PRINT / MAIL (r7.2.1: WITH BOM)
  // ========================================================================
  function exportToCsv() {
    ...
    // ✅ r7.2.1: BOM for Japanese support
    const BOM = '\uFEFF';
...
<script>window.print();</script>
  ...
  // ========================================================================
  // TOGGLE TABLE LOCK (r7.1.5: HORIZONTAL SCROLL LOCK)
  // ========================================================================
  ...
  // ========================================================================
  // OPEN PANEL (r7.1.5 LAYOUT + r7.2.1 SCROLL FIX)
  // ========================================================================
  function openPanel() {
    console.log('[TeflonManager r7.2.2] openPanel called');
    ...
    // ✅ r7.1.5 layout with r7.2.1 scroll fix
    const html = `
...
<div class="table-wrapper table-locked" style="flex:1;overflow:auto;min-height:160px;">
...
<div class="vi">Gửi email</div>
  ...
  function bindPanelEvents() {
    ...
    if (mailBtn) {
      mailBtn.addEventListener('click', mailView);
    }
  ...
  // ========================================================================
  function INIT() {
    console.log('[TeflonManager r7.2.2] Initializing...');
    console.log('[TeflonManager] ✅ Ultimate Version: r7.2.1 scroll + r7.1.5 features');
    console.log('[TeflonManager] ✅ CSS: teflon-manager-r7.2.2.css');
    injectStyles();
    initNavButton();
  ...
  console.log('[TeflonManager r7.2.2] Module loaded');

})();

/**
 * ========================================================================
 * END OF FILE - teflon-manager-r7.2.2.js
 * ========================================================================
 * 
 * SUMMARY OF FEATURES:
 * 
 * ✅ From r7.2.1 (Scroll Fix):
 *    - Vertical scroll: flex:1 + overflow:auto + min-height:160px
 *    - CSV BOM support: \uFEFF for Japanese characters
 *    - Process manager retry logic: ensureTeflonProcessManagerReady()
 * 
 * ✅ From r7.1.5 (All Features):
 *    - Nav badge with kanji: 発 (approved) / 承 (pending) / 中 (processing)
 *    - Horizontal scroll lock: toggleTableLock() + .table-locked class
 *    - Filter 2-line layout: .filter-grid with .filter-line-1 and .filter-line-2
 *    - Clickable mold name: data-action="open-process"
 *    - Clickable status badge: data-action="view-status"
 *    - Lock/Unlock button: Show/hide columns dynamically
 * 
 * ✅ New in r7.2.2:
 *    - CSS reference fixed: teflon-manager-r7.2.2.css
 *    - All scroll issues resolved
 *    - Complete integration of r7.1.5 + r7.2.1
 *    - Production ready
 * 
 * DEPENDENCIES:
 *    - CSS: teflon-manager-r7.2.2.css (REQUIRED)
 *    - window.DataManager (molds, teflonlog, employees)
 *    - window.TeflonProcessManager (optional)
 * 
 * USAGE:
 *    1. Include CSS: <link rel="stylesheet" href="teflon-manager-r7.2.2.css">
 *    2. Include JS: <script src="teflon-manager-r7.2.2.js"></script>
 *    3. Add nav button: <button id="nav-teflon-btn">テフロン管理</button>
 *    4. Module auto-initializes on DOMContentLoaded
 * 
 * PUBLIC API:
 *    - window.TeflonManager.openPanel() - Open the panel manually
 *    - window.TeflonManager.closePanel() - Close the panel
 *    - window.TeflonManager.refreshData() - Reload data
 *    - window.TeflonManager.toggleTableLock() - Toggle column visibility
 *    - window.TeflonManager.updateNavBadge() - Update badge display
 * 
 * ========================================================================
 */
