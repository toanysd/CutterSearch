...
class MobilePanelController {
  ...
  handleBottomNavTab(tab) {
    switch(tab) {
      ...
      case 'location':
        // MOBILE ONLY: Toggle floating action bar
        if (!this.isMobile) {
          console.log('Location tab - iPad mode: No action');
          return;
        }
        
        const floatingBar = document.getElementById('floating-action-bar');
        if (floatingBar && floatingBar.classList.contains('active')) {
          // Hide action bar ONLY (do not touch detail panel)
          floatingBar.classList.remove('active');
          document.body.classList.remove('floating-actions-active');
          console.log('✅ Location tab: Action bar hidden (detail panel remains)');
        } else {
          // Show action bar ONLY
          floatingBar.classList.add('active');
          document.body.classList.add('floating-actions-active');
          console.log('✅ Location tab: Action bar shown (detail panel remains)');
        }
        break;
  ...
  showFloatingActionBar() {
    ...
    if (floatingBar) {
      ...
      if (bottomNav) {
        const locationTab = bottomNav.querySelector('[data-tab="location"]');
        if (locationTab) {
