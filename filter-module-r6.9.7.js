...
(function () {
  ...
  const FILTER_FIELDS = [
    ...
    { id: 'rackLayerId', label: '棚位置 | Giá-Tầng (ID)', get: (it) => it.rackLayerInfo?.RackLayerID || '' },
    { id: 'rackLocation', label: '棚-段位置/Vị trí kệ', get: it => it.rackInfo?.RackLocation || '' },
    { id: 'rackId', label: '棚番号/Mã kệ', get: it => it.rackLayerInfo?.RackID || '' },
    { id: 'layerNum', label: '棚の段/Tầng', get: it => it.rackLayerInfo?.RackLayerNumber || '' },
