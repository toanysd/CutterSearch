...
const LocationManager = {
        ...
        <section class="loc-status">
          ...
          <div class="location-status-badges">
            ...
            <div class="location-badge-row">
              <span class="location-badge-label">ID / Mã</span>
              <div class="badge badge-mold">${moldID}</div>
            </div>

            <!-- Mold Name -->
            <div class="location-badge-row">
              <span class="location-badge-label">名称 / Tên</span>
              <div class="badge badge-mold-name">${moldName}</div>
            </div>

            <!-- Current Rack-Layer -->
            <div class="current-location-group">
              <span class="location-badge-label">現在位置 / Vị trí hiện tại</span>
              <div class="badge-group">
                <div class="badge badge-rack">${rackDisplay}</div>
                <span class="badge-sep">-</span>
                <div class="badge badge-layer">Tầng ${layerDisplay}</div>
              </div>
              <div class="rack-location">
                <span class="loc-label">保管場所 / Vị trí kho:</span>
  ...
  bindModalEvents: function(item, racksList, rackLayersList) {
    ...
    if (confirmBtn) {
      confirmBtn.addEventListener('click', () => this.saveRecord(item, rackLayersList));
    }
  ...
  const employeeValue = employeeSelect?.value?.trim(); // ✅ THÊM
    ...
    setTimeout(() => LocationManager.close(), 300);
