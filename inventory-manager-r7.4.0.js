...
(function() {
    ...
    window.SelectionManager = window.SelectionManager || {
        // ---------------------------------------------------------
        // R7.4.0: Mirror selection -> InventoryState.selectedItems
        // + phát thêm event legacy 'selectionchanged' để module cũ nhận
        // ---------------------------------------------------------
        _syncInventoryMirror() {
        try {
            if (!window.InventoryState) return;
            const src = Array.isArray(SelectionState.items) ? SelectionState.items : [];
            // mirror format: [{id,type,data?}] (giữ tương thích bulk audit)
            window.InventoryState.selectedItems = src.map(it => ({
            id: String(it.id),
            type: it.type || 'mold',
            data: it.data || it.item || null
            }));
        } catch (e) {
            console.warn('[SelectionManager] _syncInventoryMirror error', e);
        }
        },
        _emitSelectionChanged(extraDetail = null) {
        const detail = extraDetail || { items: [...SelectionState.items] };

        // event chuẩn
        document.dispatchEvent(new CustomEvent('selection:changed', { detail }));

        // event legacy (một số đoạn code cũ đang listen nhầm)
        document.dispatchEvent(new CustomEvent('selectionchanged', { detail }));
        },
        ...
        this._syncInventoryMirror();
        this._emitSelectionChanged({ items: [...SelectionState.items] });
        ...
        this.updateDomHighlights();
        this._syncInventoryMirror();
        this._emitSelectionChanged({ items: [] });
    ...
    window.InventoryState = {
        ...
        sessionId: null,        // R7.4.0: "Q" cho kiểm kê nhanh
        sessionActive: false,   // R7.4.0: đang trong phiên kiểm kê nhanh?
        multiSelectEnabled: false, // R7.4.0: cho phép multi-select trong session
    ...
    window.InventoryManager = {
        ...
        // =====================================================================
        // R7.4.0: Quick audit session button (棚卸Q | Kiểm kê ngay)
        // =====================================================================
        setupQuickAuditButton() {
        // tránh tạo trùng
        if (document.getElementById('inv-quick-session-btn')) return;

        const btn = document.createElement('button');
        btn.id = 'inv-quick-session-btn';
        btn.type = 'button';
        btn.textContent = '棚卸Q | Kiểm kê ngay';
        btn.style.cssText = `
            position: fixed;
            right: 16px;
            bottom: 88px;
        ...
        btn.addEventListener('click', (e) => {
            e.preventDefault();
            e.stopPropagation();
            this.startQuickAuditSession();
        });

        document.body.appendChild(btn);
        },

        startQuickAuditSession() {
        // đảm bảo có operator
        if (!window.InventoryState.operator) {
            this.setDefaultOperator();
        }
        if (!window.InventoryState.operator) {
            alert('担当者が未設定です | Chưa có nhân viên thực hiện');
            return;
        }

        // bật inventory + bulk + session Q
        window.InventoryState.active = true;
        window.InventoryState.bulkMode = true;

        window.InventoryState.auditEnabled = true;       // quick
        window.InventoryState.sessionId = 'Q';
        window.InventoryState.sessionActive = true;
        window.InventoryState.multiSelectEnabled = true;

        // bật selection mode (ưu tiên SelectionManager)
        ...
        // phát event đồng bộ cho UIRenderer
        this.emitInventoryModeChanged({ reason: 'quick-session-q' });

        // áp dụng filter/sort hiện tại (nếu có)
        try { this.applyFilters(); } catch (e) {}

        console.log('[InventoryManager] ✅ Quick session Q started');
        },

        // =====================================================================
        // R7.4.0: Emit đồng bộ mode cho các module khác (UIRenderer...)
        // - UIRenderer patch đang nghe: 'inventory:modeChanged' + 'inventorymodeChanged'
        // =====================================================================
        emitInventoryModeChanged(extra = {}) {
        const st = window.InventoryState || {};
        const detail = {
            ...st,
            // keys cho UIRenderer sync selection mode
            inventoryOn: !!st.active,
            sessionActive: !!st.sessionActive,
            multiSelectEnabled: !!st.multiSelectEnabled,
            ...extra
        };

        document.dispatchEvent(new CustomEvent('inventory:modeChanged', { detail }));
        document.dispatchEvent(new CustomEvent('inventorymodeChanged', { detail })); // lowercase m
        document.dispatchEvent(new CustomEvent('inventoryModeChanged', { detail })); // legacy/camel
        },
        ...
        // Dispatch events
        // R7.4.0: emit đầy đủ keys cho sync selection mode
        window.InventoryState.sessionActive = !!(window.InventoryState.active && window.InventoryState.bulkMode);
        window.InventoryState.multiSelectEnabled = !!window.InventoryState.bulkMode;
        this.emitInventoryModeChanged();
        ...
        this.renderMenubarToggle();
        this.setupQuickAuditButton(); // R7.4.0
        ...
        toggleOff() {
           ...
           // }
            ...
            // Dispatch event
            window.InventoryState.sessionId = null;
            window.InventoryState.sessionActive = false;
            window.InventoryState.multiSelectEnabled = false;
            this.emitInventoryModeChanged();
        ...
        showBulkTools() {
            ...
            if (counterText) counterText.textContent = String(count);
            ...
            // bind 1 lần (tránh trùng)
            if (!this._bulkCounterBound) {
            this._bulkCounterBound = true;
            document.addEventListener('selection:changed', updateCounter);
            document.addEventListener('selectionchanged', updateCounter); // legacy
            }
