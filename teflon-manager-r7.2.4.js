/**
 * ========================================================================
 * teflon-manager-r7.2.4.js - Quản lý mạ Teflon (AUTO-RELOAD FIX)
 * ========================================================================
 *
 * Based on: r7.2.3
 *
 * FIXED in r7.2.4 (2025-12-18 12:19 JST):
 * ✅ DATA LOADING: Luôn gọi buildRows() khi mở modal (như r7.1.1)
 * ✅ BADGE AUTO-UPDATE: Polling mỗi 30s để cập nhật badge
 * ✅ CROSS-DEVICE SYNC: Lắng nghe storage/custom event để đồng bộ
 * ✅ All r7.2.3 features preserved (pagination, scroll fix)
 *
 * Dependencies:
 * - window.DataManager (molds, teflonlog, employees)
 * - window.TeflonProcessManager (optional)
 *
 * Updated: 2025-12-18 12:19 JST
 * ========================================================================
 */
...
(function () {
  ...
  // Pagination state (r7.2.3)
  let currentPage = 1;
  const rowsPerPage = 50;

  // ✅ r7.2.4: Auto-reload state
  ...
  // ✅ r7.2.4: Data hash for change detection
  function calculateDataHash(rows) {
    if (!rows || rows.length === 0) return '0';
    const str = rows.map(r => `${r.MoldID}:${r.TeflonStatusKey}:${r.RequestedDate}`).join('|');
    // Simple hash
    let hash = 0;
  ...
  // ========================================================================
  function buildRows() {
    console.log('[TeflonManager r7.2.4] buildRows called');
    ...
    // ✅ r7.2.4: Update data hash for change detection
    const newHash = calculateDataHash(rows);
    const dataChanged = (newHash !== lastDataHash);
    lastDataHash = newHash;

    console.log(`[TeflonManager] Built ${rows.length} rows (hash: ${newHash}, changed: ${dataChanged})`);
  ...
  function updateSummaryBar() {
    ...
    if (filteredRows.length === 0) {
      summary.textContent = '表示フィルター別に依頼状況を表示します。 | Hiển thị trạng thái theo danh mục.';
      return;
    }
  ...
  function startAutoReload() {
    ...
    console.log('[TeflonManager r7.2.4] Starting auto-reload (30s interval)');

    autoReloadInterval = setInterval(() => {
      console.log('[TeflonManager r7.2.4] Auto-reload check...');

      // Check if DataManager has new data
      if (window.DataManager && typeof window.DataManager.loadAllData === 'function') {
        // Just rebuild from current data in memory
        const dataChanged = buildRows();
        
        if (dataChanged) {
          console.log('[TeflonManager r7.2.4] Data changed, updating display...');
  ...
  function stopAutoReload() {
    if (autoReloadInterval) {
      ...
      console.log('[TeflonManager r7.2.4] Stopped auto-reload');
    }
  }

  // ========================================================================
  // CROSS-DEVICE SYNC (✅ r7.2.4 NEW)
  // ========================================================================
  function setupCrossDeviceSync() {
    // Listen for storage events (cross-tab communication)
    window.addEventListener('storage', (e) => {
      if (e.key === 'teflonDataUpdated') {
        console.log('[TeflonManager r7.2.4] Storage event detected, reloading data...');
        buildRows();
        updateNavBadge();
    ...
    window.addEventListener('teflon-data-updated', () => {
      console.log('[TeflonManager r7.2.4] Custom event detected, reloading data...');
      buildRows();
      updateNavBadge();
    ...
    console.log('[TeflonManager r7.2.4] Cross-device sync listeners attached');
  }
  ...
  function injectStyles() {
    if (document.getElementById('teflon-manager-r7-2-4-styles')) return;

    const link = document.createElement('link');
    link.id = 'teflon-manager-r7-2-4-styles';
    link.rel = 'stylesheet';
    link.href = 'teflon-manager-r7.2.4.css';
    document.head.appendChild(link);

    console.log('[TeflonManager r7.2.4] CSS injected');
  }

  // ========================================================================
  // RENDER TABLE (r7.2.3: WITH PAGINATION)
  // ========================================================================
  function renderTable() {
    ...
    if (!filteredRows || filteredRows.length === 0) {
      tbody.innerHTML = `<tr><td colspan="8" style="padding:16px;text-align:center;color:#888;">データがありません | Không có dữ liệu</td></tr>`;
      return;
    }
    ...
    pageRows.forEach(row => {
      ...
      html += `<tr data-mold-id="${escapeHtml(row.MoldID)}" class="${rowClass}" style="cursor:pointer;border-bottom:1px solid #eee;">
        <td class="mold-name-cell" style="padding:8px 10px;min-width:120px;max-width:250px;">
          <a href="javascript:void(0)" data-action="open-process" title="${escapeHtml(moldName)}">${escapeHtml(moldName)}</a>
        </td>
        <td style="padding:8px 10px;text-align:center;">
          <span class="status-badge ${statusClass}" data-action="view-status" title="${escapeHtml(statusShort)}">${escapeHtml(statusShort)}</span>
        </td>
        <td style="padding:8px 10px;text-align:center;">
  ...
  function openDetailModal(row) {
    ...
    const html = `<div id="teflon-detail-modal" class="modal-overlay" style="position:fixed;top:0;left:0;width:100%;height:100%;background:rgba(0,0,0,0.5);z-index:12000;display:flex;align-items:center;justify-content:center;">
      <div class="modal-content" style="background:#fff;width:90%;max-width:720px;border-radius:8px;box-shadow:0 4px 20px rgba(0,0,0,0.3);max-height:90vh;display:flex;flex-direction:column;">
        ...
        <div class="modal-body" style="padding:12px 16px;overflow-y:auto;flex:1;">
          <table style="width:100%;border-collapse:collapse;font-size:12px;">
            ...
            ${detailRow('発送日 / Ngày gửi', escapeHtml(formatDate(row.SentDate)))}
            ...
            ${detailRow('コーティングタイプ / Loại coating', escapeHtml(row.CoatingType || '-'))}
            ...
            ${detailRow('備考 / Ghi chú', escapeHtml(row.TeflonNotes || '-'))}
            ${detailRow('データ元 / Nguồn', escapeHtml(row.source === 'teflonlog' ? 'teflonlog.csv' : 'molds.csv'))}
          </table>
        </div>
  ...
  function updateNavBadge() {
    ...
    if (hasApproved) {
      ...
      badge.textContent = '発'; // 承認済 → 発送待ち
    } else if (hasPending) {
      badge.classList.add('tef-nav-badge-pending');
      badge.textContent = '承'; // 承認待ち
    } else if (hasProcessing) {
      badge.classList.add('tef-nav-badge-processing');
      badge.textContent = '中'; // 加工中
    } else {
      badge.classList.add('tef-nav-badge-hidden');
      badge.textContent = '';
    }

    console.log('[TeflonManager r7.2.4] Badge updated:', {
      hasApproved,
      hasPending,
      hasProcessing,
      display: badge.textContent || 'hidden'
    });
  }
  ...
  function exportToCsv() {
    ...
    const BOM = '\uFEFF';
    const headers = ['No', '金型名', '状態', '依頼日', '依頼者', '発送日', '受入日', '担当者', 'メモ'];
  ...
  function mailView() {
    ...
    const headerLine = padRight('No', 4) + ' ' +
      padRight('金型名', 20) + ' ' +
      padRight('状態', 14) + ' ' +
      padRight('依頼日', 13) + ' ' +
      padRight('発送日', 13) + ' ' +
  ...
  function refreshData() {
    console.log('[TeflonManager r7.2.4] Refreshing data...');

    const btn = document.getElementById('teflon-refresh-btn');
    if (btn) {
      btn.disabled = true;
      btn.innerHTML = '<div class="jp">読込中...</div><div class="vi">Loading...</div>';
    }

    if (window.DataManager && typeof window.DataManager.loadAllData === 'function') {
      window.DataManager.loadAllData()
        .then(() => {
          console.log('[TeflonManager r7.2.4] Data refreshed');
          buildRows();
          applyFilterAndSort();
          updateNavBadge();
          ...
          // ✅ r7.2.4: Notify other tabs
          try {
            localStorage.setItem('teflonDataUpdated', Date.now().toString());
            window.dispatchEvent(new CustomEvent('teflon-data-updated'));
          } catch (e) {
            console.warn('[TeflonManager] Could not dispatch update event:', e);
          }
        })
        .catch(err => {
          console.error('[TeflonManager r7.2.4] Refresh failed:', err);
          alert('データ更新に失敗しました。 | Lỗi khi làm mới dữ liệu.');
  ...
  function toggleTableLock() {
    ...
    if (lockBtn) {
      if (isTableLocked) {
        ...
        lockBtn.title = 'クリックして全カラムを表示 | Bấm để hiện tất cả cột';
      } else {
        lockBtn.classList.add('unlocked');
        lockBtn.innerHTML = '<div class="jp">ロック</div><div class="vi">Lock</div>';
        lockBtn.title = 'クリックしてカラムを非表示 | Bấm để ẩn cột';
      }
    }
  ...
  // ========================================================================
  // OPEN PANEL (✅ r7.2.4: ALWAYS BUILD ROWS)
  // ========================================================================
  function openPanel() {
    console.log('[TeflonManager r7.2.4] openPanel called');
    ...
    // ✅ r7.2.4: ALWAYS build rows (like r7.1.1)
    console.log('[TeflonManager r7.2.4] Building rows on open (always)...');
    buildRows();

    // Updated HTML with pagination buttons
    const html = `<div id="teflon-panel" class="checkio-panel teflon-panel" style="display:block;">
      ...
      <div class="tef-summary">
        表示フィルター別に依頼状況を表示します。 | Hiển thị trạng thái theo danh mục.
      </div>
      ...
      <div class="checkio-body panel-body">
        <div class="filter-row">
          ...
          <div class="filter-grid">
            ...
            <div class="filter-line filter-line-1">
              ...
              <div class="filter-control">
                <select id="teflon-status-filter">
                  ...
                  <option value="pending">承認待ち | Chờ duyệt</option>
            ...
            <div class="filter-line filter-line-2">
              ...
              <button id="teflon-lock-btn" class="tef-btn tef-btn-lock" type="button" title="クリックして全カラムを表示 | Bấm để hiện tất cả cột">
                <div class="jp">解除</div>
        ...
        <div class="table-wrapper table-locked">
          <table id="teflon-table" class="teflon-table">
            <thead>
              <tr>
                <th data-sort="MoldName">金型名<br><span style="font-size:10px;opacity:0.9;">Tên khuôn</span><span class="sort-indicator"></span></th>
                <th data-sort="TeflonStatusKey">状態<br><span style="font-size:10px;opacity:0.9;">Trạng thái</span><span class="sort-indicator"></span></th>
                <th data-sort="RequestedDate">依頼日<br><span style="font-size:10px;opacity:0.9;">Ngày y/c</span><span class="sort-indicator">▼</span></th>
                <th data-sort="RequestedByName">依頼者<br><span style="font-size:10px;opacity:0.9;">Người y/c</span><span class="sort-indicator"></span></th>
                <th data-sort="SentDate" class="col-hidden-locked">発送日<br><span style="font-size:10px;opacity:0.9;">Ngày gửi</span><span class="sort-indicator"></span></th>
                <th data-sort="ReceivedDate" class="col-hidden-locked">受入日<br><span style="font-size:10px;opacity:0.9;">Ngày nhận</span><span class="sort-indicator"></span></th>
                <th data-sort="SentByName" class="col-hidden-locked">担当者<br><span style="font-size:10px;opacity:0.9;">Phụ trách</span><span class="sort-indicator"></span></th>
                <th class="col-hidden-locked">メモ<br><span style="font-size:10px;opacity:0.9;">Ghi chú</span></th>
              </tr>
            </thead>
        ...
        <div class="tef-actions">
          ...
          <button id="teflon-prev-btn" class="tef-btn tef-btn-blue" type="button" title="前ページ | Trang trước">
            <div class="jp">◀ 前</div>
            <div class="vi">Previous</div>
          </button>
          <button id="teflon-next-btn" class="tef-btn tef-btn-blue" type="button" title="次ページ | Trang sau">
            <div class="jp">次 ▶</div>
    ...
    console.log('[TeflonManager r7.2.4] Panel opened with pagination');
  }
  ...
  function closePanel() {
    ...
    console.log('[TeflonManager r7.2.4] Panel closed');
  }
  ...
  function initNavButton() {
    ...
    console.log('[TeflonManager r7.2.4] Nav button bound');
  }
  ...
  // ========================================================================
  function INIT() {
    console.log('[TeflonManager r7.2.4] Initializing...');
    console.log('[TeflonManager r7.2.4] ✅ AUTO-RELOAD: 30s interval');
    console.log('[TeflonManager r7.2.4] ✅ CROSS-DEVICE SYNC: enabled');
    console.log('[TeflonManager r7.2.4] ✅ CSS: teflon-manager-r7.2.4.css');
    ...
    // ✅ r7.2.4: Start auto-reload
    startAutoReload();

    // ✅ r7.2.4: Setup cross-device sync
  ...
  console.log('[TeflonManager r7.2.4] Module loaded (AUTO-RELOAD FIX)');

})();

/* ========================================================================
   END OF FILE - teflon-manager-r7.2.4.js
   ========================================================================

   CHANGELOG r7.2.4 (2025-12-18 12:23 JST)
   -----------------------------------------------------------------------
   ✅ AUTO-RELOAD FIX:
   - ALWAYS call buildRows() when openPanel() (like r7.1.1)
   - Badge auto-update: polling every 30 seconds
   - Cross-device sync: listen to storage & custom events
   
   ✅ DATA SYNCHRONIZATION:
   - calculateDataHash() for change detection
   - startAutoReload() / stopAutoReload() methods
   - setupCrossDeviceSync() for multi-device support
   - localStorage broadcast when data changes
   
   ✅ ALL r7.2.3 FEATURES PRESERVED:
   - Pagination: 50 rows per page
   - Scroll fix: proper height constraint
   - Navigation badge with kanji (発/承/中)
   - Lock/unlock horizontal scroll
   - Filter 2-line layout
   - All export/print/mail functions
   
   ✅ COMPARISON WITH r7.1.1:
   Module          Data Loading      Badge Update    Scroll   Pagination
   ----------------------------------------------------------------
   r7.1.1          ✅ Always load    ✅ Auto         ❌       ❌
   r7.2.3          ❌ Only once      ❌ Manual       ✅       ✅
   r7.2.4          ✅ Always load    ✅ Auto         ✅       ✅
   
   DEPENDENCIES:
   - CSS: teflon-manager-r7.2.4.css
   - window.DataManager (molds, teflonlog, employees)
   - window.TeflonProcessManager (optional)
   
   USAGE:
   1. Include CSS: <link rel="stylesheet" href="teflon-manager-r7.2.4.css">
   2. Include JS: <script src="teflon-manager-r7.2.4.js"></script>
   3. Add nav button: <button id="nav-teflon-btn">...</button>
   ...
   - window.TeflonManager.refreshData()   - Reload data manually
   - window.TeflonManager.goToPage(n)     - Jump to page n
   - window.TeflonManager.startAutoReload() - Start auto-reload
   - window.TeflonManager.stopAutoReload()  - Stop auto-reload
