// server-r6.9.7.js - V7.7.7-r6.9.7 FIX COMPLETE!
// 
...
/**
 * POST /api/audit
 * Lưu audit records vào statuslogs.csv
 * Hỗ trợ single record hoặc batch (array)
 */
app.post('/api/audit', async (req, res) => {
  try {
    const records = Array.isArray(req.body) ? req.body : [req.body];
    
    console.log('[Server] Received audit batch:', records.length);
    
    // Validate required fields
    for (const record of records) {
      if (!record.itemId || !record.itemType || !record.auditDate) {
        return res.status(400).json({ 
          success: false, 
          error: 'Missing required fields: itemId, itemType, auditDate' 
        });
      }
    }
    
    // Read current CSV
    const csvPath = DATAPATH_PREFIX + 'statuslogs.csv';
    const fileData = await getGitHubFile(csvPath);
    ...
    // Generate new IDs
    let maxId = Math.max(0, ...existingRecords.map(r => parseInt(r.StatusLogID) || 0));
    
    // Create new CSV lines
    const newLines = [];
    for (const record of records) {
      maxId++;
      const moldId = record.itemType === 'mold' ? record.itemId : '';
      const cutterId = record.itemType === 'cutter' ? record.itemId : '';
      const timestamp = new Date(record.auditDate).toISOString();
      ...
      const csvLine = [
        ...
        record.operator || '',
        '', // DestinationID
        record.notes || 'Inventory audit',
        record.auditDate.split('T')[0] // YYYY-MM-DD
      ].join(',');
    ...
    // Append to CSV
    const updatedLines = [header, ...lines.slice(1), ...newLines];
    const updatedContent = updatedLines.join('\n');
    
    await updateGitHubFile(csvPath, updatedContent, fileData.sha, `Add ${records.length} audit record(s)`);
    
    console.log('[Server] Saved', records.length, 'audit records to CSV');
    
    res.json({ 
      success: true, 
      message: `Saved ${records.length} audit record(s)`,
      records: newLines.length
    });
    
  } catch (error) {
    console.error('[Server] Audit save error:', error);
    res.status(500).json({ 
      success: false, 
      error: error.message 
    });
