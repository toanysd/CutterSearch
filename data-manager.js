...
(function () {
  ...
  async function fetchCSVWithFallback(filename, required) {
    ...
    try {
      ...
      const res = await fetch(GITHUB_BASE_URL + filename, {
        // When opening from file://, CORS is typically fine for GET;
        // keep it simple to ensure text() is accessible.
        method: 'GET',
        cache: 'no-cache',
    ...
    try {
      const resLocal = await fetch('./' + filename, { method: 'GET', cache: 'no-cache' });
      if (resLocal.ok) {
