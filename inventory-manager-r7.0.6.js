    ...
    window.InventoryManager = {
        ...
        loadAuditHistoryFromStatusLogs() {
            try {
                ...
                // Filter AUDIT records only
                const auditRecords = statusLogs.filter(log => {
                    const status = (log.Status || '').toUpperCase();
                    return status === 'AUDIT' || status.includes('AUDIT');
                });
                
                console.log(`[InventoryManager] 📊 Loading ${auditRecords.length} audit records from statuslogs...`);
                
                // Populate auditHistory cache
                ...
                console.log('[InventoryManager] ✅ Audit history loaded from statuslogs:', 
                    Object.keys(window.InventoryState.auditHistory).length, 'records');
                
                // Refresh UI badges
                document.dispatchEvent(new CustomEvent('inventoryrefreshBadges'));
                
            } catch (e) {
                console.error('[InventoryManager] Failed to load audit history from statuslogs:', e);
            }
        },
