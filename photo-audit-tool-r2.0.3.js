...
PHOTO AUDIT TOOL - R2.0.3
写真監査ツール / Công cụ kiểm tra ảnh khuôn

CHANGES FROM R2.0.2:
- iPhone optimized: Fixed camera overlay z-index and visibility
- Dropdown highlight working (yellow highlight like Location Manager)
- Preview info selector fixed (#id instead of .class)
- File input handling fixed (no auto-close)
- Modal positioning improved for mobile
- Search results display correctly

Architecture:
- Integration: DataManager (direct access)
- Settings Screen: Searchable dropdown + manual input + highlight
...
const PhotoAuditUtils = {
  ...
  // Highlight matched text - NEW for R2.0.3
  highlightMatch: (text, query) => {
    if (!query || !text) return text;
    
    const escapedText = text.toString();
    const escapedQuery = query.toString().trim();
    
    if (!escapedQuery) return escapedText;
    
    // Case-insensitive search - escape special regex characters
    const regex = new RegExp(`(${escapedQuery.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')})`, 'gi');
    return escapedText.replace(regex, '<span class="pa-search-highlight">$1</span>');
  },
...
const PhotoAuditTool = {
  ...
  init() {
    ...
    console.log('📷 [PhotoAuditTool v2.0.3] Initializing...');
    ...
    console.log('✅ [PhotoAuditTool v2.0.3] Initialized successfully!');
  ...
  buildSettingsScreen() {
    ...
    header.innerHTML = `
      <div class="pa-settings-title">
        ...
        <div>
          <div><span class="label-ja">写真監査</span> <span class="label-vi">Photo Audit</span></div>
      ...
      <div class="pa-section">
        ...
        <div class="pa-search-wrapper">
          <input 
            ...
            id="pa-mold-search"
            placeholder="コード or 名前を入力 / Mã hoặc tên khuôn..."
            autocomplete="off"
          />
      ...
      <div class="pa-section">
        ...
        <div class="pa-dimensions-grid">
          <div class="pa-dim-input-wrapper">
            ...
            <input type="number" class="pa-input pa-dim-input" id="pa-dim-length" placeholder="Dài" step="0.1" />
          </div>
          <div class="pa-dim-input-wrapper">
            <label class="pa-dim-label">W (mm)</label>
            <input type="number" class="pa-input pa-dim-input" id="pa-dim-width" placeholder="Rộng" step="0.1" />
          </div>
          <div class="pa-dim-input-wrapper">
            <label class="pa-dim-label">H (mm)</label>
            <input type="number" class="pa-input pa-dim-input" id="pa-dim-depth" placeholder="Cao" step="0.1" />
          </div>
        </div>
        <p class="pa-hint">
      ...
      <div class="pa-section">
        ...
        <div class="pa-search-wrapper">
          <input 
            ...
            placeholder="名前を入力 / Tên nhân viên..."
            autocomplete="off"
          />
  ...
  bindSettingsEvents() {
    ...
    this.els.moldSearchInput.addEventListener('focus', () => this.showMoldDropdown());
    ...
    this.els.moldToggle.addEventListener('click', () => this.toggleMoldDropdown());
    ...
    this.els.employeeSearchInput.addEventListener('focus', () => this.showEmployeeDropdown());
    ...
    this.els.employeeToggle.addEventListener('click', () => this.toggleEmployeeDropdown());
    ...
    document.addEventListener('click', (e) => {
      if (
        !this.els.moldSearchInput.contains(e.target) &&
      ...
      if (
        !this.els.employeeSearchInput.contains(e.target) &&
  ...
  filterMoldDropdown(query) {
    const { escapeHtml: e, createElement: ce, highlightMatch } = PhotoAuditUtils;
    ...
    if (q) {
        // FIX: Split query into terms for flexible matching
        ...
        results = this.state.molds.filter(m => {
            ...
            const searchText = `${code} ${name} ${designCode} ${displayName}`;
    ...
    if (results.length === 0) {
      this.els.moldSearchResults.innerHTML = `
        <div class="pa-search-empty">
          ...
          <div class="empty-hint">Using manual input: "${e(query)}"</div>
        </div>
      `;
    ...
    results.forEach((mold) => {
      ...
      const code = mold.MoldCode || mold.displayCode || '';
      const name = mold.MoldName || mold.displayName || '';
      const dimensions = mold.displayDimensions;
      const location = mold.displayRackLocation;

      // Highlight matched text
      const highlightedCode = highlightMatch(e(code), q);
      const highlightedName = highlightMatch(e(name), q);
      const highlightedDimensions = dimensions ? highlightMatch(e(dimensions), q) : '';
      const highlightedLocation = location ? highlightMatch(e(location), q) : '';

      item.innerHTML = `
        <div class="pa-search-item-main">
          <div class="pa-search-item-code">${highlightedCode}</div>
          <div class="pa-search-item-name">${highlightedName}</div>
        </div>
        ${dimensions ? `<div class="pa-search-item-meta"><span class="meta-dim"><i class="fas fa-ruler-combined"></i>${highlightedDimensions}</span></div>` : ''}
        ${location ? `<div class="pa-search-item-meta"><span class="meta-loc"><i class="fas fa-map-marker-alt"></i>${highlightedLocation}</span></div>` : ''}
      `;
  ...
  selectMold(mold) {
    ...
    console.log('✅ [PhotoAuditTool] Mold selected:', { code: mold.MoldCode, id: mold.MoldID });
  },
    ...
    if (this.state.selectedMold) {
      ...
      this.els.moldSelectedDisplay.innerHTML = `
        <div class="pa-selected-tag manual">
          ...
          <span>Manual: ${e(this.state.manualMoldInput)}</span>
        </div>
      `;
    } else {
      this.els.moldSelectedDisplay.innerHTML = '';
  ...
  filterEmployeeDropdown(query) {
    ...
    results.forEach((emp) => {
      ...
      const name = emp.EmployeeNameShort || emp.EmployeeName || '';
      const division = emp.Division || '';
      
      const highlightedName = highlightMatch(e(name), q);
      const highlightedDivision = division ? highlightMatch(e(division), q) : '';

      item.innerHTML = `
        <div class="pa-search-item-main">
          <div class="pa-search-item-code"><i class="fas fa-user"></i> ${highlightedName}</div>
          ${division ? `<div class="pa-search-item-meta">${highlightedDivision}</div>` : ''}
        </div>
      `;
  ...
  updateEmployeeSelection(name) {
    ...
    console.log('✏️ [PhotoAuditTool] Manual employee set:', this.state.selectedEmployee);
  },
  ...
  updateEmployeeDisplay() {
    ...
    if (this.state.selectedEmployee) {
      ...
      const prefix = emp.isManual ? 'Manual: ' : '';
  ...
  loadDimensionsForMold(mold) {
    console.log('📐 [PhotoAuditTool] Loading dimensions for mold:', { id: mold.MoldID, code: mold.MoldCode });
  ...
  addRecipient() {
    ...
    if (this.state.recipients.includes(email)) {
      this.toast('既に追加されています / Already added', 'error');
      return;
    }
    ...
    this.renderRecipients();
    this.toast('追加されました / Recipient added', 'success');
  },

  removeRecipient(email) {
    this.state.recipients = this.state.recipients.filter((e) => e !== email);
    this.renderRecipients();
    this.toast('削除されました / Recipient removed', 'info');
  },
  ...
  renderRecipients() {
    ...
    this.state.recipients.forEach((email) => {
      ...
      `;
      tag.querySelector('.pa-recipient-remove').addEventListener('click', () => this.removeRecipient(email));
  ...
  validateSettings() {
    ...
    if (!this.state.selectedMold && !this.state.manualMoldInput) {
      this.toast('金型を選択または入力してください / Please select or enter mold', 'error');
      return false;
    }

    // Check employee - accept manual input
    if (!this.state.selectedEmployee || !this.state.selectedEmployee.name) {
      this.toast('担当者を選択または入力してください / Please select or enter employee', 'error');
      return false;
    }

    // Check recipients
    if (this.state.recipients.length === 0) {
      this.toast('送信先を追加してください / Please add recipients', 'error');
      return false;
    }
  ...
  async handleFileSelect(file) {
    ...
    try {
      ...
      // FIX R2.0.3: Use setTimeout to prevent auto-close
      setTimeout(() => {
        this.closeSettings();
        setTimeout(() => {
  ...
  openSettings(preFillData = null) {
    ...
    // Auto-focus on mold search after a short delay
    setTimeout(() => {
  ...
  preFillMoldData(mold) {
      console.log('[PhotoAuditTool] Pre-filling mold data:', { code: mold.MoldCode, id: mold.MoldID });
      ...
      if (this.els.moldSearchInput) {
          // FIX: Chỉ fill MoldCode, không thêm " - MoldName"
          this.els.moldSearchInput.value = mold.MoldCode || '';
          this.els.moldSearchInput.classList.remove('manual-input');
          this.els.moldSearchInput.classList.add('auto-selected');
      }
  ...
  buildCameraScreen() {
    ...
    view.innerHTML = `
      ...
      <div class="pa-camera-loading" id="pa-camera-loading">
        ...
        <div class="pa-camera-loading-text">Loading camera...</div>
      </div>

      <div class="pa-camera-error" id="pa-camera-error">
        <i class="fas fa-exclamation-triangle"></i>
        <div class="pa-camera-error-title">Camera Error</div>
        <div class="pa-camera-error-message">Cannot access camera</div>
      </div>
    `;
    ...
    previewControls.innerHTML = `
      ...
      <div class="pa-preview-actions">
        <button class="pa-btn-preview pa-btn-retake" id="pa-btn-retake">
          ...
          <span class="btn-label-ja">撮り直し</span>
    ...
    // FIX R2.0.3: Use #id selector instead of .class
  ...
  async startCamera() {
    ...
    try {
      ...
      this.toast('カメラにアクセスできません / Camera access failed', 'error');
    }
  },
  ...
  async flipCamera() {
    ...
    await this.startCamera();
    this.toast('カメラ切り替え / Camera flipped', 'info');
  },
  ...
  async capturePhoto() {
    if (!this.state.stream) {
      this.toast('カメラが起動していません / Camera not active', 'error');
      return;
    }
  ...
  async showPreview() {
    ...
    // Update preview info - FIX R2.0.3: Show full info
  ...
  async sendPhotoAudit() {
    ...
    try {
      ...
      this.toast('送信完了 / Email sent successfully', 'success');
  ...
  buildEmailPayload(fileName, photoUrl) {
    ...
    const date = PhotoAuditUtils.formatDateJP();
    const subject = `${moldCode} - ${date} 写真検査レポート / Photo Audit`;
  ...
  buildEmailBody(moldCode, mold, employee, dims, photoUrl, fileName) {
    ...
    const e = PhotoAuditUtils.escapeHtml;
...
<head>
  ...
  <style>
    ...
    .label { font-weight: bold; color: #6a5acd; }
    .photo { margin: 20px 0; text-align: center; }
    .photo img { max-width: 100%; height: auto; border: 2px solid #6a5acd; }
...
<body>
  <div class="container">
    <div class="header">
      <h1>📷 写真検査レポート / Photo Inspection Report</h1>
      <p>${e(datetime)}</p>
    </div>
    <div class="content">
      <div class="info-row">
        <span class="label">金型コード / Mold Code:</span> ${e(moldCode)}
      </div>
      <div class="info-row">
        <span class="label">金型名 / Mold Name:</span> ${e(mold?.MoldName || 'Manual Input')}
      </div>
      <div class="info-row">
        <span class="label">寸法 / Dimensions:</span> 
        L: ${e(dims.length || '-')} mm × W: ${e(dims.width || '-')} mm × H: ${e(dims.depth || '-')} mm
      </div>
      <div class="info-row">
        <span class="label">担当者 / Employee:</span> ${e(employee?.name || 'Unknown')}
      </div>
      <div class="photo">
        <img src="${e(photoUrl)}" alt="Mold Photo" />
      </div>
    </div>
    <div class="footer">
      <p>This email was automatically generated by Photo Audit Tool v2.0.3</p>
      <p>📁 File: ${e(fileName)}</p>
    </div>
  </div>
</body>
  ...
  ======================================== */
  bindGlobalHooks() {
    // Listen for trigger from navbar or other modules
    document.addEventListener('open-photo-audit', (e) => {
      console.log('🎯 [PhotoAuditTool] Triggered from external:', e.detail);
      this.openSettings(e.detail);
    });

    // Listen for trigger from MobileDetailModal
    document.addEventListener('photoAudit:trigger', (e) => {
      console.log('🎯 [PhotoAuditTool] Triggered from MobileDetailModal:', e.detail);
      this.openSettings(e.detail);
    });
  },
...
======================================== */
window.PhotoAuditTool = {
  openSettings: (preFillData) => PhotoAuditTool.openSettings(preFillData),
  closeSettings: () => PhotoAuditTool.closeSettings(),
  init: () => PhotoAuditTool.init()
};
...
console.log('📦 [PhotoAuditTool v2.0.3] Module loaded');
