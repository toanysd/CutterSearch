/**
 * teflon-process-manager-r7.1.0.js
 * ==========================================================
 * Module nghiệp vụ mạ Teflon nâng cao - inspired by VBA workflow
 * テフロン加工依頼・完了処理モジュール（改良版）
 *
 * NEW in r7.1.0:
 *  ✅ Migration Helper: Tự động phát hiện dữ liệu cũ trong molds.TeflonCoating
 *  ✅ Quick Transitions: Shortcuts chuyển trạng thái nhanh (giống VBA)
 *  ✅ Smart Auto-fill: Tự động điền form theo context hiện tại
 *  ✅ Status Workflow Hints: Hiển thị gợi ý bước tiếp theo
 *  ✅ Default Employee: Cấu hình nhân viên mặc định
 *
 * Backend:
...
(function () {
  ...
  function determineNextStatus(currentStatusKey) {
    // Xác định trạng thái tiếp theo dựa trên workflow
    if (!currentStatusKey || currentStatusKey === '') return 'pending';
    if (currentStatusKey === 'pending') return 'sent';
  ...
  const TeflonProcessManager = {
    INIT() {
      console.log('TeflonProcessManager r7.1.0 loaded (VBA-inspired workflow)');
    },
    ...
    async openPanel(item) {
      ...
      const html =
        ...
        // NEW: Current Status Display (hiển thị đầu tiên)
        currentStatusHTML +
        ...
        '        <div class="badge-row"><span class="badge-label">現在の保管先 / Công ty lưu trữ</span><div class="badge badge-company">' + escapeHtml(storageCompanyName || '-') + '</div></div>' +
        '        <div class="badge-row"><span class="badge-label">ラック位置 / Vị trí kệ</span><div class="badge badge-rack">' + escapeHtml(rackLayer || '-') + '</div></div>' +
        '      </div>' +
        '    </section>' +
      ...
      // NEW: Smart Auto-fill dựa trên trạng thái hiện tại
    ...
    _buildCurrentStatusDisplay(currentStatusKey, historyLogs) {
      if (!currentStatusKey || currentStatusKey === '') {
        ...
        return (
          ...
          '    未処理 / Chưa xử lý' +
          '  </div>' +
          '  <p style="margin:8px 0 0 0;font-size:12px;color:#6b7280;">このコンテンツはまだテフロン加工に出荷されていません。<br>Khuôn này chưa được gửi đi mạ Teflon.</p>' +
          '</section>'
        );
      }
      ...
      if (currentStatusKey === 'pending') {
        ...
        statusDescription = 'このコンテンツはテフロン加工の承認待ちです。<br>Khuôn đang chờ phê duyệt để gửi đi mạ.';
        ...
        statusDescription = 'このコンテンツは現在テフロン加工業者で加工中です。<br>Khuôn đang được mạ tại nhà cung cấp.';
        if (lastLog) {
        ...
        statusDescription = 'このコンテンツのテフロン加工は完了しました。<br>Khuôn đã được mạ Teflon hoàn tất.';
        if (lastLog) {
      ...
      return (
        ...
        (dateInfo ? '<div style="margin-top:8px;padding:8px;background:rgba(255,255,255,0.6);border-radius:4px;font-size:12px;color:' + statusTextColor + ';">' + dateInfo + '</div>' : '') +
        '</section>'
      );
    },

    // NEW: Build Quick Actions UI
    _buildQuickActions(currentStatusKey, nextStatusKey) {
      if (!currentStatusKey || currentStatusKey === '') {
        // Trạng thái trống: không hiển thị quick actions
        return '';
      }

      let html = '<section class="tefproc-quick-actions" style="background:#f0fdf4;border:1px solid #86efac;border-radius:8px;padding:12px;margin-bottom:16px;">';
      html += '<h4 style="margin:0 0 8px 0;font-size:14px;color:#166534;">⚡ クイックアクション / Quick Actions</h4>';

      if (currentStatusKey === 'pending') {
        // Chờ phê duyệt → gửi đi mạ
        html +=
          '<button type="button" id="tefproc-quick-send" class="btn-quick" style="width:100%;background:#10b981;color:#fff;border:none;padding:10px;border-radius:6px;font-weight:600;cursor:pointer;">' +
          '📦 今日出荷確認 / Xác nhận gửi đi mạ hôm nay' +
          '</button>';
      } else if (currentStatusKey === 'sent') {
        // Đang mạ → hoàn tất
        html +=
          '<button type="button" id="tefproc-quick-complete" class="btn-quick" style="width:100%;background:#3b82f6;color:#fff;border:none;padding:10px;border-radius:6px;font-weight:600;cursor:pointer;">' +
          '✅ 受入完了確認 / Xác nhận đã nhận hôm nay' +
          '</button>';
      }

      html += '</section>';
      return currentStatusKey === 'completed' ? '' : html;
    },
    ...
    _applySmartAutoFill(currentStatusKey, nextStatusKey, historyLogs) {
      ...
      if (!currentStatusKey || currentStatusKey === '') {
        // Trạng thái trống → mặc định "Chờ phê duyệt"
        if (statusSelect) statusSelect.value = 'pending';
      } else if (currentStatusKey === 'pending') {
        // Chờ phê duyệt → mặc định "Đang mạ"
        ...
        // Đang mạ → chuyển sang tab "Xác nhận đã mạ"
        // (logic sẽ xử lý khi user click tab)
      }
    ...
    _bindEvents(item, companies, employees, teflonlog, currentStatusKey, nextStatusKey) {
      ...
      // Auto ExpectedDate khi thay đổi SentDate
      const statusSelect = document.getElementById('tefproc-status');
      const statusPill = document.getElementById('tefproc-status-pill');
      const sentDateEl = document.getElementById('tefproc-sent-date');
      const expDateEl = document.getElementById('tefproc-expected-date');
      ...
      if (quickSendBtn) {
        quickSendBtn.addEventListener('click', this._handleQuickSend.bind(this, item, companies, employees));
      }

      if (quickCompleteBtn) {
        quickCompleteBtn.addEventListener('click', this._handleQuickComplete.bind(this, item, companies, employees));
      }

      // Gửi đi mạ (full form)
      const saveSendBtn = document.getElementById('tefproc-save-send');
      if (saveSendBtn) {
        saveSendBtn.addEventListener('click', this._handleSendSubmit.bind(this, item, companies, employees));
      }

      // Xác nhận đã mạ (full form)
      const confirmBtn = document.getElementById('tefproc-confirm-complete');
      if (confirmBtn) {
        confirmBtn.addEventListener('click', this._handleCompleteSubmit.bind(this, item, companies, employees));
      }
    ...
    async _handleQuickSend(item, companies, employees) {
      ...
      const tefEntry = {
        ...
        SupplierID: DEFAULT_SUPPLIER_ID,
        CoatingType: '',
        Reason: 'Quick send via Quick Action',
        TeflonCost: '',
        Quality: '',
      ...
      try {
        ...
        try {
          ...
          if (!stRes.ok || !stJson.success) {
            console.warn('[QuickSend] statuslogs warning', stJson);
          }
        } catch (e) {
          console.warn('[QuickSend] statuslogs error', e);
        }

        // 3) Cập nhật in-memory
        if (data) {
        ...
        if (dm && typeof dm.recompute === 'function') {
          try {
            ...
            console.warn('[QuickSend] recompute error', e);
          }
        }
    ...
    async _handleQuickComplete(item, companies, employees) {
      ...
      try {
        ...
        try {
          ...
          const stJson = await stRes.json();
          if (!stRes.ok || !stJson.success) {
            console.warn('[QuickComplete] statuslogs warning', stJson);
          }
        } catch (e) {
          console.warn('[QuickComplete] statuslogs error', e);
        }

        // 3) Cập nhật molds.csv
        try {
          ...
          if (!updRes.ok || !updJson.success) {
            console.warn('[QuickComplete] molds.csv warning', updJson);
          }
        } catch (e) {
          console.warn('[QuickComplete] molds.csv error', e);
        }

        // 4) Cập nhật in-memory
        if (data) {
          if (!Array.isArray(data.teflonlog)) data.teflonlog = [];
          data.teflonlog.unshift(tefEntry);
        ...
        if (dm && typeof dm.recompute === 'function') {
          try {
            ...
            console.warn('[QuickComplete] recompute error', e);
          }
        }
        ...
        // Hỏi có muốn cập nhật vị trí không
    ...
    // ============================
    // Full Form Handlers (giữ nguyên logic r7.0.9)
    // ============================
      ...
      try {
        ...
        if (statusEntry) {
          try {
            ...
            const stJson = await stRes.json();
            if (!stRes.ok || !stJson.success) {
              console.warn('[Send] statuslogs warning', stJson);
            }
          } catch (e) {
        ...
        if (dm && typeof dm.recompute === 'function') {
          try {
            ...
            console.warn('[Send] recompute error', e);
          }
        }
    ...
    async _handleCompleteSubmit(item, companies, employees) {
      ...
      try {
        ...
        try {
          ...
          const stJson = await stRes.json();
          if (!stRes.ok || !stJson.success) {
            console.warn('[Complete] statuslogs warning', stJson);
          }
        } catch (e) {
        ...
        try {
          ...
          const updJson = await updRes.json();
          if (!updRes.ok || !updJson.success) {
            console.warn('[Complete] molds.csv warning', updJson);
          }
        } catch (e) {
        ...
        if (dm && typeof dm.recompute === 'function') {
          try {
            ...
            console.warn('[Complete] recompute error', e);
          }
        }
  ...
  // Bridge: lắng nghe triggerTeflon
