  ...
  const UIRenderer = {
    ...
    renderQuickCards(items) {
      ...
      items.slice(0, 100).forEach((item, idx) => {
        ...
        // R7.0.5: iPad-style status badge with sync icon
        const auditBadge = `<span class="checkin-status-badge ${statusBadgeClass}">
            <span class="badge-text">${statusBadgeText}</span>
            <span class="sync-icon synced" title="Đã đồng bộ">✓</span>
        </span>`;
