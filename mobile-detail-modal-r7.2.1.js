...
triggerPhotoAudit(item, type) {
  ...
  setTimeout(() => {
    ...
    if (!item) {
        alert('Lỗi: Không có dữ liệu vật phẩm để kiểm kê');
        return;
    }

    const itemId = (itemType === 'cutter') ? (item.CutterID || item.id) : (item.MoldID || item.id);
    if (!itemId) {
        alert('Lỗi: Không tìm thấy ID (MoldID/CutterID) để kiểm kê');
        return;
    }

    // 1) Đóng modal ngay lập tức (đúng yêu cầu “kiểm kê xong là đóng detail”)
    this.hide();

    // 2) Ghi kiểm kê
    try {
        if (window.InventoryManager && typeof window.InventoryManager.recordAudit === 'function') {
        // recordAudit đã tự gọi auditSingle + tự phát inventoryauditRecorded (badge/UI cập nhật)
        await window.InventoryManager.recordAudit(String(itemId), itemType);
        return;
        }

        // Fallback: nếu InventoryManager chưa sẵn thì bắn event để InventoryManager (fallback handler) xử lý
        document.dispatchEvent(new CustomEvent('inventory:auditSingle', {
        detail: { itemId: String(itemId), itemType }
        }));
        document.dispatchEvent(new CustomEvent('inventoryauditSingle', {
        detail: { itemId: String(itemId), itemType }
        }));
    } catch (err) {
